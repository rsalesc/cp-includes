...
namespace hashing {
  ...
  struct RollingHash {
    ...
    struct Hash {
      ...
      Hash(mint_type y, int n) : x(y), n(n) {
        assert(n >= 0);
      }
    ...
    Hash substr(int i, int j) const {
      return (*this)(i, j);
    }

    Hash all() const {
      return Hash(hs.back(), size());
    }
  ...
  struct BidirectionalRollingHash {
    ...
    template<
      typename Container,
      typename enable_if<HasBidirectionalIterator<Container>::value>::type* = nullptr>
    BidirectionalRollingHash(const Container& container,
                             const shared_ptr<base_type>& base)
      : BidirectionalRollingHash<T, Mods...>(container.begin(), container.end(), base) {}

    template<
      typename Iterator,
      typename enable_if<IsBidirectionalIterator<Iterator>::value>::type* = nullptr>
  ...
  struct HashProvider {
    ...
    explicit HashProvider(base_type base) 
      : cat(make_shared<base_type>(base))  {}
  ...
  template<typename R>
  struct PowerHashProvider : HashProvider<R> {
  ...
  using Roll32 = RollingHash<int32_t, Mods...>;

  template<int64_t ...Mods>
  using Roll64 = RollingHash<int64_t, Mods...>;
