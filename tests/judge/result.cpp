...
struct SegtreeBeats {
  ...
  T query(int no, int l, int r, int i, int j, const Folder &folder) {
    ...
    if (i <= l && r <= j)
      return folder(t[no]);
    int mid = (l + r) / 2;
    return folder(query<T>(no << 1, l, mid, i, j, folder),
...
struct Node {
  ...
  Node operator+(const Node &rhs) const { return Node(max(mx, rhs.mx)); }
};
...
void solve() {
  ...
  auto hld = graph::make_range_hld<
      SegtreeBeats<Node, seg::CombineFolder<Node>, Pushdown>>(f);
