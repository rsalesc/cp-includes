...
namespace treap {
  ...
  struct DefaultNode {
    ...
    inline static void combine(DefaultNode& no,
                               DefaultNode* ln,
                               DefaultNode* rn,
  ...
  template<typename T,
           typename Combiner = EmptyCombiner,
           typename Pushdown = EmptyPushdown,
  ...
  struct TreapManager {
    ...
    inline void update(vnode no) {
      if(!manager.has(no)) return;
      combiner_fn(manager.ref(no),
                  manager.ptr(manager.left(no)),
                  manager.ptr(manager.right(no)));
    }

    template<typename Checker>
    bool check(vnode no, const Checker& checker) {
      if(!manager.has(no)) return false;
      return checker(manager.ref(no),
          manager.ptr(manager.left(no)),
          manager.ptr(manager.right(no)));
    }
    ...
    vnode bsearch_last_impl(vnode no, const Checker& checker) {
      push(no);
      if(!manager.has(no)) return null();
      if(check(manager.right(no), checker))
        return bsearch_last_impl(manager.right(no), checker);
      else if(check(no, checker)) return no;
      else return bsearch_last_impl(manager.left(no), checker);
    }

    template<typename Folder, typename Checker>
    vnode bsearch_last_impl(vnode no, const Folder& folder, const Checker& checker) {
      push(no);
      if(!manager.has(no)) return null();
    }
    ...
    pair<vnode, vnode> split(vnode no, const Checker& checker) {
      push(no);
      if(!manager.has(no)) return {null(), null()};
    ...
    pair<vnode, vnode> split_on_node(vnode no, const Checker& checker) {
      return split(no, [&checker](const TreapNode& no, TreapNode* ln, TreapNode* rn) {
        return checker(no);
      });
    }
