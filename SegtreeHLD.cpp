...
template <typename S, typename T, typename Folder> struct QueryIssuer {
  ...
  T operator()(int i) const { seg.template query_element<T>(i, folder); }
  T operator()(int i, int j) const {
    return seg.template query<T>(i, j, folder);
  }
};

template <typename S, typename T, typename Folder> struct QueryLifter {
...
template <typename S, typename Updater> struct UpdateIssuer {
  ...
  UpdateIssuer(S &seg, const Updater &updater) : seg(seg), updater(updater) {}
...
template <typename S, typename Updater> struct SplashIssuer {
  S &seg;
  const Updater &updater;
  SplashIssuer(S &seg, const Updater &updater) : seg(seg), updater(updater) {}
...
template <typename S, typename Beater> struct BeatIssuer {
  S &seg;
  const Beater &beater;
  BeatIssuer(S &seg, const Beater &beater) : seg(seg), beater(beater) {}
...
template <typename S, typename G> struct RangeHLD : HLD<G> {
  ...
  inline T query_vertex(int u, const Folder &folder) {
    return this->template query_on_vertex(
        u, range::QueryIssuer<S, T, Folder>(seg, folder));
  }
